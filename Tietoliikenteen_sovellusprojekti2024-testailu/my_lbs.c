...
#define MAX_NOTIFICATION_SIZE 32
...
//Lisätty
// STEP 14 - Muokkaa my_lbs_send_sensor_notify-funktiota

int my_lbs_send_sensor_notify(uint32_t sensor_value_x, uint32_t sensor_value_y, uint32_t sensor_value_z, uint32_t direction_value)
{
    if (!notify_mysensor_enabled) {
        return -EACCES;
    }
/*
	// Yhdistä arvot taulukkoon
    uint32_t data[4] = {sensor_value_x, sensor_value_y, sensor_value_z, direction_value};

    // Lähetä kaikki arvot kerralla
    return bt_gatt_notify(NULL, &my_lbs_svc.attrs[7], data, sizeof(data));
*/

	// Buffer to hold the ASCII representation of the data
    char data[128];
    snprintf(data, sizeof(data), "%u,%u,%u,%u", sensor_value_x, sensor_value_y, sensor_value_z, direction_value);

    // Send the data as ASCII text
   return bt_gatt_notify(NULL, &my_lbs_svc.attrs[7], data, strlen(data));
}
